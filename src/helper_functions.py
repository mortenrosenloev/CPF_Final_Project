...
def calculate_meta_model_returns(
    df,
    tp_multiplier,
    sl_multiplier,
    lookahead,
    ...
    """
    Simulates strategy execution based on predicted meta labels 
    and evaluates returns.

    This function assumes that the model gives trade 
    predictions (`pred == 1`) and combines this with directional 
    signals to enter trades. It manages active
    positions and evaluates exit conditions: TP, SL, or timeout. 
    Realistic handling of entry/exit prices and transaction costs 
    is included.

    Parameters:
    - df (pd.DataFrame): Must include columns:
        - 'signal': trade direction (1 for long, -1 for short)
        - 'ATRr_14': ATR volatility estimate
        - 'open', 'close', 'high', 'low': price data
        - 'r': log returns per bar
        - 'pred': predicted meta label (1 = trade, 0 = no trade)
    - tp_multiplier (float): Take-profit distance (as multiple of ATR).
    - sl_multiplier (float): Stop-loss distance (as multiple of ATR).
    - lookahead (int): Max bars to hold trade before timeout.
    - tc (float, optional): Transaction cost per round-trip trade 
        (in log returns). Default is 0.

    Returns:
    - df (pd.DataFrame): Data enriched with:
        - 'pos': position over time
        - 's': strategy returns without transaction costs
        - 's_tc': strategy returns with transaction costs
        - 'comment': trade commentary per bar (entry/exit reason)
    - trades_df (pd.DataFrame): Details of each closed trade 
        (entry/exit, pnl, reason, etc.)
    """
    
    df = df.copy()

    signal = df['signal'].values
    atr = df['ATRr_14'].values
    open_price = df['open'].values
    close_price = df['close'].values
    high_price = df['high'].values
    low_price = df['low'].values
    ...
    strategy_returns_with_tc = np.zeros(len(df))
    comments = np.full(len(df), np.nan, dtype=object)

    active_positions = [] 
    closed_positions = [] 

    for i in range(len(df)):
        pos_change = 0  # Entries or exit counter for each line
        ...
        if pred[i] == 1:
            ...
            if direction == 1:
                take_profit = entry_price + tp_multiplier * atr[i]
                stop_loss = entry_price - sl_multiplier * atr[i]
            else:
                take_profit = entry_price - tp_multiplier * atr[i]
                stop_loss = entry_price + sl_multiplier * atr[i]
                
            strategy_returns_with_tc[i] += - 0.5 * tc
            trade_id = len(closed_positions) + len(active_positions) + 1
            comments[i] = f'Id: {trade_id} | entry {direction}'
        ...
        for p in active_positions.copy():
            trade_id = p['id']
            ...
            if direction == 1:  # Long position
                if low_price[i] <= stop_loss:
                    exit_price = stop_loss
                    if bars_held == 1:  # Special case for the first bar
                        bar_pnl = np.log(exit_price / entry_price)
                    else:
                        bar_pnl = np.log(exit_price / close_price[i - 1])
                    bar_pnl_tc = bar_pnl - 0.5 * tc
                    reason = f'Id: {trade_id} | long_sl'
                    exit_trade = True
                elif high_price[i] >= take_profit:
            ...
            if not exit_trade:
                if bars_held >= lookahead:  # Timeout
                    exit_price = close_price[i]
                    bar_pnl = np.log(exit_price / close_price[i - 1]) * direction
                    bar_pnl_tc = bar_pnl - 0.5 * tc
                    reason = f'Id: {trade_id} | timeout'
                    exit_trade = True
                if not exit_trade:  # Stay in the trade and update trade values
                    if bars_held == 1:  # Special case for the first bar
                        bar_pnl = direction * np.log(
                            close_price[i] / entry_price
                        )
                    else:
                        bar_pnl = direction * returns[i]
                    bar_pnl_tc = bar_pnl
                    bars_held += 1

            # Update returns
            strategy_returns[i] += bar_pnl
            strategy_returns_with_tc[i] += bar_pnl_tc
            ...
            if exit_trade:
                pos_change -= direction  # We have exited the position
                p.update({
                    'exit_index': i,
        ...
        if i > 0:
            positions[i] += positions[i - 1] + pos_change
        else:
            positions[i] = pos_change

    df['pos'] = positions
    df['s'] = strategy_returns
    df['s_tc'] = strategy_returns_with_tc
    df['comment'] = comments
