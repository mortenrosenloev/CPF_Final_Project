class MetaModelBacktester:
    ...
    def _init_data(self):
        """
        Prepares internal data structures, arrays, and containers for the backtest.
        """
    ...
    def _run(self):
        """
        Core loop of the backtest. Iterates through the data, handles entries and exits, and tracks performance.
        """
        ...
        for i in range(len(self.df)):
            ...
            if self.pred[i] == 1:
                ...
                # Adjust strategy returns for the given bar with the entry spread 
                self.strategy_returns_tc[i] -= 0.5 * self.tc
                
                self.comments[i] = f"Id: {trade['id']} | entry {trade['direction']}"
    ...
    def _finalize(self):
        """
        Finalizes the results after backtesting, populates the result DataFrame and trades list.
        """
    ...
    def _check_exit_conditions(self, p, i):
        ...
        Returns:
            tuple: exit_trade (bool), bar_pnl (float), bar_pnl_tc (float), reason (str), exit_price (float).
        """        
        ...
        if not exit_trade:
            if bars_held >= self.lookahead:
                ...
                bar_pnl = direction * np.log(exit_price / self.close_price[i - 1])
                reason = f'Id: {trade_id} | timeout'
                exit_trade = True
            else:
                if bars_held == 1:
                    bar_pnl = direction * np.log(self.close_price[i] / entry_price)
                else:
    ...
    def plot_performance(self):
        """
        Plots cumulative returns of buy-and-hold vs strategy (with/without trade cost).
        """        
        
        plt.figure(figsize=(10, 6))
        plt.title('Strategy returns', fontsize=14)
        plt.plot(self.results['r'].cumsum().apply(np.exp), label='Buy-and-hold')
        plt.plot(self.results['s'].cumsum().apply(np.exp), label='Strategy excl. trading cost')
        plt.plot(self.results['s_tc'].cumsum().apply(np.exp), label='Strategy excl. trading cost')
        plt.legend()
        plt.show();
    ...
    def plot_trades_histogram(self):    
        ...
        plt.figure(figsize=(10, 6))
        plt.hist(self.trades_df['pnl_tc'], bins=30, color='steelblue', edgecolor='black')
